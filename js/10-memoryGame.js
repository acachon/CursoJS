//Variables globales
    ...
    var tempo = new Timer;      //Guardo el temporizador
    var contadorGlobal=0;       //Acumulo el numero de parejas exitosas
...
// Funciones locales
    ...
    // Inicializo el tablero y barajo las imagenes entre las casillas
        ...
        //3. Arranco el timer y creo un listener para actualizarlo y muestro el timer
        tempo.reset();
        tempo.addEventListener('secondsUpdated', function (e) {
            document.getElementById("p2").innerHTML=tempo.getTimeValues().toString();
        });
    }
    ...
    // Pongo las dos imagenes en oculto en mi nuevaPareja, y reinicializo nuevaPareja
        for (var i=0; i<nuevaPareja.length;i++){
    ...
    //TODO: cambiar la logica de juego aqui para que tenga que encontrar todas las parejas 
        ...
        if (imgVisibles==1){
            ...
            if (ganador){
                //Acumulo una pareja mas y compruebo si he acabado (contadorGlobal)
                contadorGlobal++;
                if (contadorGlobal==Math.floor(document.getElementById("input1").value/2)) {
                    document.getElementById("p1").innerHTML = "Enhorabuena, CAMPEON !!";  
